...
// ─── SVG 라인 차트 ─────────────────────────────────────────
function LineChart({ data, series, t }) {
...
export default function MyActivityPage() {
  ...
  useEffect(() => {
    fetch("http://localhost:3010/activity/stats", {
      headers: { Authorization: `Bearer ${localStorage.getItem("accessToken")}` },
    })
      .then(r => r.json())
      .then(res => { if (res.success) setData(res.data); else setError("데이터를 불러오지 못했습니다."); })
      .catch(err => { console.error("Fetch 에러:", err); setError("서버와 연결할 수 없습니다."); })
      .finally(() => setLoading(false));
  }, []);
  ...
  const { summary, monthlyActivity, followMonthlyActivity, topTags, saveStats, followStats, recentPosts, topLikedPosts, topViewedPosts, topScrappedPosts } = data;
  ...
  const topPostTabs = [
    ...
    { key: "scraps",  label: "스크랩",   Icon: TurnedInNotOutlined },
  ];
    ...
    <div style={{ minHeight: "100vh", background: t.bg }}>
      <div style={{ maxWidth: 720, margin: "0 auto", padding: "32px 16px 64px", display: "flex", flexDirection: "column", gap: 20, fontFamily: '"Plus Jakarta Sans", "Noto Sans KR", sans-serif' }}>
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
          ...
          <MiniStatBox value={saveStats.scrapCount}       label="스크랩"  Icon={TurnedInNotOutlined}    t={t} />
          ...
          <>
            ...
            <Section title="팔로워 변동 추이" Icon={TrendingUpOutlined} t={t}>
              {followMonthlyActivity && followMonthlyActivity.length > 0
                ? <FollowChart data={followMonthlyActivity} t={t} />
        ...
        {activeTab === "top" && (
          <Section title="인기 게시글" Icon={EmojiEventsOutlined} t={t}>
            ...
            {topPostTab === "likes" && (
              <TopPostList posts={topLikedPosts} metric="likeCount" metricLabel="개" Icon={FavoriteBorderOutlined} color="coral" mode={mode} t={t} />
            )}
            {topPostTab === "views" && (
              <TopPostList posts={topViewedPosts} metric="viewCount" metricLabel="회" Icon={VisibilityOutlined} color="blue" mode={mode} t={t} />
            )}
            {topPostTab === "scraps" && (
              <TopPostList posts={topScrappedPosts} metric="scrapCount" metricLabel="회" Icon={TurnedInNotOutlined} color="purple" mode={mode} t={t} />
            )}
          </Section>
        )}
